...
import { RecycleScroller as RawRecycleScroller } from 'vue-virtual-scroller'
...
const RecycleScroller = RawRecycleScroller as any
...
function renderGraph(opts: RenderGraphOpts = {}) {
  ...
  if (!container) return
  if (!rawData || !rawData.nodes?.length) {
    container.innerHTML = ''
    return
  }
  const data: WikiGraphData = rawData
