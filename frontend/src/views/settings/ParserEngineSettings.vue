    ...
    >
      <div v-if="currentEngine" class="drawer-content">
        ...
        <div class="form-item" v-if="currentEngine && (hasConfigFields(currentEngine.Name) || currentEngine.Name === 'builtin')">
          <label class="form-label">{{ $t('settings.parser.testConnection', '연결 테스트') }}</label>
          <div class="api-test-section">
            <t-button variant="outline" :loading="checking" @click="onCheck">
              {{ $t('settings.parser.testConnection', '연결 테스트') }}
            </t-button>
  ...
  try {
    ...
    if (currentEngine.value) {
      if (currentEngine.value.Name === 'builtin') {
        if (connected.value) {
          checkMessage.value = t('settings.parser.checkSuccess', '연결 테스트에 성공했습니다')
          saveSuccess.value = true
        } else {
          checkMessage.value = t('settings.parser.checkFailed', '연결 테스트에 실패했습니다')
        ...
        if (updatedEngine) {
          if (updatedEngine.Available) {
            ...
            checkMessage.value = updatedEngine.UnavailableReason || t('settings.parser.checkFailed', '연결 테스트에 실패했습니다')
            saveSuccess.value = false
          }
        } else {
          checkMessage.value = t('settings.parser.checkFailed', '엔진 상태를 확인할 수 없습니다')
          saveSuccess.value = false
        }
      }
    } else {
      checkMessage.value = t('settings.parser.checkDoneStatusUpdated', '검사가 완료되었고 상태가 갱신되었습니다')
      saveSuccess.value = true
    }

    setTimeout(() => { checkMessage.value = '' }, 3000)
  } catch (e: any) {
    checkMessage.value = e?.message || t('settings.parser.checkFailed', '연결 테스트에 실패했습니다')
    saveSuccess.value = false
  } finally {
