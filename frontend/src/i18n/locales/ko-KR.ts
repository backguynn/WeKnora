export default {
  ...
  webSearchSettings: {
    ...
    providerDescPlaceholder: "예: 테스트 환경용",
    engineIdLabel: "엔진 ID",
  ...
  auth: {
    ...
    logout: "로그아웃",
    oidcLogin: "OIDC로 로그인",
    oidcLoginWithProvider: "{provider}로 로그인",
    redirectingToOIDC: "인증 제공자로 이동하는 중...",
    orContinueWith: "또는 다음 방식으로 계속하기",
    oidcLoginFailed: "OIDC 로그인 실패",
    oidcStateMismatch: "OIDC 상태 검증에 실패했습니다. 다시 시도해 주세요",
    username: "사용자명",
    email: "이메일",
    password: "비밀번호",
  ...
  agent: {
    ...
    editor: {
      ...
      webFetchEnabled: "페이지 내용 자동 가져오기",
  ...
  tenant: {
    ...
    api: {
      ...
      desktopBindPublicLabel: "로컬 네트워크에서 API 접근 허용",
      desktopBindPublicDescription:
        "활성화하면 내장 서버가 0.0.0.0에서 수신하여 같은 LAN의 다른 기기에서도 아래 주소로 API에 접근할 수 있습니다. 노출 범위가 커지므로 방화벽과 라우팅 규칙으로 접근을 제어하세요. 변경 후 앱을 다시 시작해야 적용됩니다.",
      desktopBindPublicSaved: "저장되었습니다. 적용하려면 앱을 다시 시작하세요.",
      desktopBindPublicSaveFailed: "저장 실패",
      lanUrlLabel: "LAN API 기본 URL",
      lanUrlDescription:
        "「로컬 네트워크에서 API 접근 허용」을 켜고 앱을 다시 시작한 뒤 표시됩니다. IP가 정확하지 않으면 시스템 네트워크 설정에 표시된 실제 IPv4 주소로 호스트 부분을 바꾸세요.",
      lanUrlCopyTitle: "LAN API URL 복사",
      lanUrlCopySuccess: "LAN API URL이 복사되었습니다",
      lanUrlUnavailable:
        "현재 모든 네트워크 인터페이스에서 수신 중이지만 LAN IPv4를 자동으로 확인하지 못했습니다. 시스템 네트워크 설정에서 로컬 IPv4를 확인한 뒤 http://내IP:포트/api/v1 형식으로 직접 사용하세요.",
    },
  },
  ...
  agentEditor: {
    ...
    desc: {
      ...
      webSearchProvider: '이 에이전트에 사용할 검색 엔진을 지정합니다. 비워두면 기본 검색 엔진을 사용합니다',
      webSearchMaxResults: '검색당 반환되는 최대 결과 수',
      webFetchEnabled: '재정렬 후 상위 웹 결과의 전체 페이지 내용을 자동으로 가져와 답변 품질을 높입니다',
      webFetchTopN: '재정렬 후 최대 몇 개의 웹 페이지를 가져올지 설정합니다',
    ...
    im: {
      ...
      wechatScanBind: "QR 코드로 위챗 연결",
      wechatScanning: "위챗으로 QR 코드를 스캔하세요",
      wechatBindSuccess: "위챗 연결이 완료되었습니다",
      wechatRebind: "다시 연결",
      wechatHint: "iOS 위챗 8.0.70 이상이 필요하며, 1:1 대화만 지원합니다",
      wechatQRExpired: "QR 코드가 만료되었습니다. 다시 시도하세요",
      wechatBinding: "연결 중...",
    },
    tools: {
      thinking: '사고',
  ...
  datasource: {
    ...
    createAndSyncSuccess: "데이터 소스가 생성되었고 동기화 작업이 제출되었습니다",
    createButSyncFailed: "데이터 소스는 생성되었지만 동기화 트리거에 실패했습니다",
    ...
    prereqTitle: "사용 전에 아래 설정을 모두 완료하세요",
    prereqBarText: "처음 사용하시나요? 클릭하여 Feishu 앱 설정 가이드를 확인하세요",
    prereqBotBrief: "앱에 '봇' 기능 추가",
    prereqBotDesc: "오픈 플랫폼 → 앱 기능 추가 → 봇 → 버전 생성 및 배포",
    prereqBotLabel: "1. 앱에 '봇' 기능을 추가하세요 (그렇지 않으면 위키에서 앱을 검색할 수 없습니다):",
    prereqBotStep1: "Feishu 오픈 플랫폼에서 앱을 열고 왼쪽의 '앱 기능 추가'를 클릭하세요",
    prereqBotStep2: "'봇'을 선택하고 추가를 확인하세요",
    prereqBotStep3: "새 버전을 생성해 배포하거나 테스트 조직에서 디버그하세요",
    prereqPermBrief: "API 권한 부여",
    prereqPermLabel: "2. 앱에 아래 API 권한을 부여하세요:",
    prereqOpenConsole: "Feishu 오픈 플랫폼에서 설정하기",
    prereqMemberBrief: "그룹 채팅을 통해 위키 멤버로 추가",
    prereqMemberDesc: "그룹 채팅 생성 → 앱을 그룹 봇으로 추가 → 그룹 채팅을 위키 멤버로 추가",
    prereqMemberLabel: "3. 그룹 채팅을 통해 앱을 위키 멤버로 추가하세요 (위키에서 앱을 직접 검색할 수 없습니다):",
    back: "이전",
    next: "다음",
    save: "저장",
    schedule30min: "30분마다",
