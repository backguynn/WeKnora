...
if [ "$start_backend" = "y" ] || [ "$start_backend" = "Y" ]; then
    ...
    log_success "백엔드가 백그라운드에서 시작됨 (PID: $BACKEND_PID)"
...
# 3. 프런트엔드 시작 여부 확인
echo ""
log_info "단계 3/3: 프런트엔드 앱 시작"
printf "%b" "${YELLOW}현재 터미널에서 프런트엔드를 시작할까요? (y/N): ${NC}"
...
if [ "$start_frontend" = "y" ] || [ "$start_frontend" = "Y" ]; then
    ...
    log_success "프런트엔드가 백그라운드에서 시작됨 (PID: $FRONTEND_PID)"
    log_info "프런트엔드 로그 확인: tail -f $PROJECT_ROOT/logs/frontend.log"
else
    log_warning "프런트엔드 시작 건너뜀"
...
log_info "접속 주소:"
echo "  - 프런트엔드: http://localhost:5173"
...
if [ -f "$PROJECT_ROOT/tmp/backend.pid" ] || [ -f "$PROJECT_ROOT/tmp/frontend.pid" ]; then
    ...
    if [ -f "$PROJECT_ROOT/tmp/frontend.pid" ]; then
        echo "  - 프런트엔드 중지: kill \$(cat $PROJECT_ROOT/tmp/frontend.pid)"
    fi
fi
